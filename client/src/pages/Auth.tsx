...
export default function AuthPage() {
  ...
  const handlePortalAccess = (portal: any) => {
    ...
    setLoading(true);
    setMockAuth(portal.userId, portal.role);
    setTimeout(() => {
  ...
  const portals = [
    {
      id: 'ngo',
      role: 'NGO' as const,
      title: 'Sahayata-NGO Login',
      desc: 'Access resource hubs and real-time maps',
      icon: '/icons/ngo.png',
      path: '/ngo',
    ...
    },
    {
      id: 'needy',
      role: 'Needy' as const,
      title: 'Sahayata-Needy Login',
      desc: 'Request immediate relief and support',
  ...
  return (
    <div className="h-screen w-screen grid lg:grid-cols-2 bg-white selection:bg-blue-100 overflow-hidden font-sans">
      ...
      <div className="h-full flex flex-col justify-center items-center p-4 lg:p-12 bg-slate-50 relative overflow-hidden">
        <div className="w-full max-w-xl space-y-6 relative z-10 px-4">
          <div className="text-center lg:text-left space-y-1">
            ...
            <p className="text-slate-400 font-medium tracking-wide">Select the appropriate node to initialize your dashboard access.</p>
          </div>
          ...
          <div className="flex flex-col gap-4">
            {portals.map((portal, idx) => (
              <motion.button
                ...
                disabled={loading}
                className={`w-full group flex items-center gap-6 p-6 bg-white border border-slate-200 rounded-[40px] transition-all transform hover:-translate-y-2 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.08)] hover:shadow-[0_30px_70px_-10px_rgba(0,0,0,0.15)] hover:border-${portal.id === 'ngo' ? 'blue' : portal.id === 'needy' ? 'emerald' : portal.id === 'doctor' ? 'rose' : 'amber'}-500/10 ${loading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
              >
                <div className="shrink-0 relative">
                  <div className={`absolute inset-0 bg-slate-100 rounded-full blur-2xl group-hover:bg-${portal.id === 'ngo' ? 'blue' : portal.id === 'needy' ? 'emerald' : portal.id === 'doctor' ? 'rose' : 'amber'}-500/10 transition-colors opacity-0 group-hover:opacity-100`}></div>
          ...
          <div className="pt-6 flex flex-col items-center gap-4 border-t border-slate-100">
            <p className="text-xs font-medium text-slate-400 tracking-wide leading-none italic">Crafted with passion for the Hack-O-Fest 2026 by</p>
        ...
        <PortalLoader loading={loading} selectedPortal={selectedPortal} />
      </div>
    </div>
  );
}
