    ...
    >
      <div className="p-8 flex items-center h-32 whitespace-nowrap overflow-hidden">
        <div 
          ...
          className="w-16 h-16 bg-blue-50 rounded-[22px] flex items-center justify-center border-2 border-white shrink-0 relative z-10 cursor-pointer active:scale-95 transition-transform p-3 shadow-inner"
        >
           <img src="/icons/NGO/logo.png" alt="Sahayata" className="w-full h-full object-contain" />
        </div>
        <AnimatePresence>
          {isOpen && (
            <motion.div 
            ...
            >
              ...
              <span className="text-xl font-black text-blue-600 tracking-tighter ml-0.5">-NGO</span>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      >
        {menuItems.map(item => {
          const isActive = (item.id === 'dashboard' && location.pathname === '/ngo') || location.pathname === item.path;
          return (
            <motion.button
              key={item.id}
            ...
            >
              <motion.div 
                layout
                className={`flex items-center transition-all duration-500 rounded-[30px] overflow-hidden relative ${isActive ? 'bg-white shadow-[0_15px_45px_-10px_rgba(37,99,235,0.15)] ring-2 ring-blue-500/5' : 'text-slate-400 hover:bg-white/70 hover:shadow-lg hover:shadow-slate-200/20'} ${isOpen ? 'w-full p-4 h-[84px]' : 'w-16 h-16 rounded-full mx-auto justify-center p-0'}`}
              >
                {isActive && (
                  <motion.div 
                    layoutId="activeIndicatorPillNGO"
                    className={`absolute inset-0 bg-gradient-to-br from-blue-50/20 to-transparent ${isOpen ? 'border-r-4 border-blue-600' : 'border-2 border-blue-600/20 rounded-full'}`}
                  />
                )}
                
                <div className={`shrink-0 flex items-center justify-center relative z-10 ${isOpen ? 'w-14 h-14' : 'w-full h-full'} ${isActive ? 'scale-110 text-blue-600' : 'text-slate-400 group-hover:text-blue-600 group-hover:scale-110 transition-all duration-300'}`}>
                ...
                {isOpen && (
                  ...
                  >
                    ...
                    {isActive && <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[10px] font-black text-blue-600 tracking-tight mt-1">Active Hub</motion.span>}
                  </motion.div>
                )}
              </motion.div>
            </motion.button>
      ...
      <div className="p-8 border-t border-slate-200">
        ...
        >
          <div className={`flex items-center justify-center transition-all relative z-10 ${isOpen ? 'w-14 h-14' : 'w-full h-full'}`}>
            <LogOut size={24} className="text-slate-400 group-hover:text-blue-600 group-hover:rotate-12 transition-all" />
          </div>
          {isOpen && (
            <motion.span 
              variants={itemVariants}
              animate={isOpen ? "open" : "closed"}
              className="ml-6 font-black text-sm text-slate-400 group-hover:text-blue-600"
            >
              End Session
            </motion.span>
          )}
